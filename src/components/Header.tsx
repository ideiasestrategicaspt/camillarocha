    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-md">
      <div className="max-w-6xl mx-auto flex items-center justify-between px-6 py-4">
        ...
        <nav className="hidden md:flex items-center gap-8 font-body text-xs tracking-[0.2em] uppercase text-muted-foreground">
          ...
          <a href="#results" className="hover:text-foreground transition-colors">{nav.results}</a>
          <a href="#about" className="hover:text-foreground transition-colors">{nav.about}</a>
          <a href="#contact" className="hover:text-foreground transition-colors">{nav.contact}</a>
        </nav>
        ...
        >
          <span className={lang === "fr" ? "text-foreground font-medium" : ""}>FR</span>
